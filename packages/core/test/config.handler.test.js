...
describe('config handler', () => {
  test('should not throw when loading an empty configuration file', async () => {
    ...
    await expect(handler(fileCallback(currentDir)('./support/empty.json'))).resolves.not.toThrow()
  })
  ...
  test('should throw when loading a configuration that extends a non-existant one', async () => {
    ...
    await expect(handler({
      extends: [
        `${currentDir}/support/non-existant.json`
      ]
    })).rejects.toThrow()
  })
  ...
  test('should throw when trying to read an invalid configuration file', async () => {
    const currentDir = dirname(fileURLToPath(import.meta.url))
    await expect(handler(fileCallback(currentDir)('./support/invalid.json'))).rejects.toThrow()
  })

  test('should throw when trying to read an invalid JSON file', async () => {
    const currentDir = dirname(fileURLToPath(import.meta.url))
    await expect(handler(fileCallback(currentDir)('./support/invalid-json.json'))).rejects.toThrow()
  })

  test('should support comments in JSON configuration file', async () => {
    const currentDir = dirname(fileURLToPath(import.meta.url))
    await expect(handler(fileCallback(currentDir)('./support/basic-commented.json'))).resolves.not.toThrow()
  })

  test('simple chain should work', async () => {
    const currentDir = dirname(fileURLToPath(import.meta.url))
    await expect(handler(fileCallback(currentDir)('./support/chain/chain1.json'))).resolves.not.toThrow()
  })
  ...
  test('simple check using the file resolver should work', async () => {
    const currentDir = dirname(fileURLToPath(import.meta.url))
    await expect(handler(fileCallback(currentDir)('./support/chain-file/chain1.json'))).resolves.not.toThrow()
  })
  ...
  test('should throw in case of infinite loop', async () => {
    const currentDir = dirname(fileURLToPath(import.meta.url))
    await expect(handler(fileCallback(currentDir)('./support/infinite-loop/chain1.json'))).rejects.toThrow()
  })
})
