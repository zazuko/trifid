          ...
          'dcat:dataset': datasetsPointer
            .map((dataset: any) => {
              ...
              if (!identifiers.value) {
                ...
                console.error(
                  `Ignoring dataset ${dataset.value} because it has no or multiple identifiers`,
                );
                return null;
              }
              ...
              const keywords = dataset
                .out(rdf.ns.dcat.keyword)
                .filter(({ term: { language } }: any) => !!language);
              ...
              const workExampleDstributions = dataset
                .out(rdf.ns.schema.workExample)
                .filter(
                  (workExample: any) =>
                    workExample.out(rdf.ns.schema.encodingFormat).terms.length > 0,
                )
                .map((workExample: any) => ({
                  'dcat:Distribution': {
                    ...
                    'dcterms:format': {
                      '@': {
                        'rdf:resource': distributionFormatFromEncoding(
                          workExample.out(rdf.ns.schema.encodingFormat),
                        ),
                      },
                    },
                  },
              ...
              const copiedDistributions = dataset
                .out(rdf.ns.dcat.distribution)
              ...
              const publishers = dataset.out(rdf.ns.dcterms.publisher).map((publisher: any) => {
              ...
              const accrualPeriodicity = dataset
                .out(rdf.ns.dcterms.accrualPeriodicity)
                .map((t: any) => {
              ...
              return {
                'dcat:Dataset': {
                  ...
                  'dcat:contactPoint': serializeBlankNode(dataset.out(rdf.ns.dcat.contactPoint), [
                    rdf.ns.vcard.Organization,
                    rdf.ns.vcard.Individual,
                  ]),
                  'dcat:theme': serializeTerm(dataset.out(rdf.ns.dcat.theme)),
                  'dcterms:language': serializeTerm(dataset.out(rdf.ns.dcterms.language)),
                  'dcterms:relation': [
                    legalBasis,
                    serializeTerm(dataset.out(rdf.ns.dcterms.relation), {
                      properties: [rdf.ns.rdfs.label],
                    }),
                  ],
                  ...
                  'dcat:distribution': [...workExampleDstributions, ...copiedDistributions],
...
const serializeTerm = (pointer: any, { properties = [] }: { properties?: NamedNode[] } = {}) => {
  return pointer.map((value: any) => {
    return (
      ...
      serializeNamedNode(value, properties) ||
      serializeBlankNode(value) ||
      {}
    );
  });
};
...
const serializeLiteral = (pointer: any): Record<string, unknown> | null => {
  ...
  if (
    term.datatype &&
    !term.datatype.equals(rdf.ns.rdf.langString) &&
    !term.datatype.equals(rdf.ns.xsd.string)
  ) {
 ...
 */
const serializeBlankNode = (
  pointer: any,
  allowedTypesArr: NamedNode[] = [],
): Record<string, unknown> | null => {
...
function serializeProperties(pointer: any) {
  const properties = rdf.termSet(
    [...pointer.dataset.match(pointer.term)]
      .map(({ predicate }) => predicate)
      .filter((term) => !term.equals(rdf.ns.rdf.type)),
  );

  return [...properties].reduce(
    (acc, property) => ({ ...acc, [shrink(property.value)]: serializeTerm(pointer.out(property)) }),
    {},
  );
}
