...
import { LabelLoader } from './labels/labelLoader.ts';
import { TrifidResourceDescription } from './web-component/TrifidResourceDescription.ts';
...
const createEntityRenderer = ({ options = {}, logger, query }: { options?: Record<string, any>; logger?: any; query?: any }) => {
  return async (req: any, { dataset, rewriteResponse, replaceIri, entityRoot, headers }: any) => {
    ...
    const foundQuad = [...dataset].find((quad: any) => quad.subject.equals(term));
    const cf = rdf.clownface({ dataset, term: foundQuad ? term : undefined } as any);
